        ...
        configureServer(server) {
          ...
          server.middlewares.use('/api/system-mode', async (req, res, next) => {
            ...
            const readSystemState = async () => {
              try {
                const raw = await fs.readFile(MODE_STATE_FILE, 'utf8')
                return normalizeSystemState(JSON.parse(raw) as Record<string, unknown>)
              } catch {
                const initial = normalizeSystemState({})
                await fs.mkdir(path.dirname(MODE_STATE_FILE), { recursive: true })
                await fs.writeFile(MODE_STATE_FILE, `${JSON.stringify(initial, null, 2)}\n`, 'utf8')
                return initial
              }
            }
            ...
            const writeSystemState = async (nextState: Record<string, unknown>) => {
              ...
              await fs.mkdir(path.dirname(MODE_STATE_FILE), { recursive: true })
              await fs.writeFile(MODE_STATE_FILE, `${JSON.stringify(normalized, null, 2)}\n`, 'utf8')
              return normalized
            }
