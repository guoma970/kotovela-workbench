...
| DEV-INTEG-02 | `server/xiugDispatch.ts` 支持飞书 Open API `sendMessage`，优先使用 `FEISHU_APP_ID` / `FEISHU_APP_SECRET` / `FEISHU_STUDY_CHAT_ID`。 |
...
| `FEISHU_STUDY_CHAT_ID` | 果果学习布置群 chat_id。 |
...
- 飞书 `sendMessage` 需要飞书应用具备向目标群发消息权限，且 `FEISHU_STUDY_CHAT_ID` 必须是已存在的果果学习布置群。
