...
const outDir = path.resolve(process.env.SCREENSHOT_DIR ?? 'screenshots/dev76')
const evidenceDir = path.resolve(process.env.EVIDENCE_DIR ?? '.evidence/dev76')
...
const apiFixtures = {
  '/api/tasks-board': readJson('dev76-tasks-board-api.json'),
  '/api/leads': readJson('dev76-leads-api.json'),
  '/api/audit-log': readJson('dev76-audit-log-api.json'),
  '/api/system-mode': JSON.parse(fs.readFileSync(path.resolve(`server/data/system-mode.${mode}.json`), 'utf8')),
  '/evidence/dev76/drift-trend.json': readJson('dev76-drift-trend.json'),
}

const shots = mode === 'internal'
  ? [
      ['/evidence-acceptance', 'DEV-76-internal-evidence-acceptance-overview.png', '[data-evidence-link="true"]'],
      ['/tasks', 'DEV-76-internal-tasks-split-evidence.png', 'body'],
      ['/leads', 'DEV-76-internal-leads-split-evidence.png', 'body'],
    ]
  : [
      ['/evidence-acceptance', 'DEV-76-opensource-evidence-isolation.png', '.page-note'],
    ]
...
const structuredLeadSignals = await page.evaluate(() => document.body.textContent?.match(/source_line|account_line|content_line/g)?.length ?? 0)
fs.writeFileSync(path.join(evidenceDir, `mode-isolation-${mode}.json`), JSON.stringify({ mode, evidence_chip_count: evidenceChipCount, drift_card_count: driftCardCount, structured_signal_mentions: structuredLeadSignals }, null, 2))

await browser.close()
console.log(`Captured DEV-76 ${mode} screenshots at ${outDir}`)
