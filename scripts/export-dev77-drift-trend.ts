...
const publicDir = path.join(root, 'public', 'evidence', 'dev77')
const evidenceDir = path.join(root, '.evidence', 'dev77')
fs.mkdirSync(publicDir, { recursive: true })
fs.mkdirSync(evidenceDir, { recursive: true })

const normalize = (value?: string) => String(value ?? '').trim()

const projects: Project[] = [{ id: 'project-1', code: 'PRJ-1', name: '言町科技工作台', owner: 'builder', ownerAgentId: 'agent-2', status: 'active', progress: 87, focus: 'room/content drift split', blockers: 0, stage: 'delivery', nextStep: 'separate source_line and content_line buckets', taskCount: 1, roomIds: ['room-2'] }]
const agents: Agent[] = [{ id: 'agent-2', code: 'AG-2', name: 'builder', role: 'builder', status: 'active', currentTask: 'DEV-77', project: '言町科技工作台', projectId: 'project-1', updatedAt: new Date().toISOString(), instanceKey: 'builder' }]
const rooms: Room[] = [{ id: 'room-2', code: 'ROOM-2', name: '言町科技运营群', status: 'active', focus: 'room/content drift split', pending: 0, channelType: 'feishu', instance: 'builder', instanceIds: ['agent-2'], mainProject: '言町科技工作台', mainProjectId: 'project-1', purpose: 'route review', recentAction: 'DEV-77 drift export' }]
const tasks: Task[] = [{ id: 'task-2', code: 'TASK-2', title: 'room/content drift export', project: '言町科技工作台', projectId: 'project-1', assignee: 'builder', assigneeAgentId: 'agent-2', executor: 'builder', executorAgentId: 'agent-2', status: 'doing', priority: 'high', updatedAt: new Date().toISOString() }]
...
fs.writeFileSync(path.join(evidenceDir, 'dev77-drift-trend.json'), JSON.stringify(enriched, null, 2))
console.log(`Exported DEV-77 drift trend to ${path.relative(root, path.join(evidenceDir, 'dev77-drift-trend.json'))}`)
