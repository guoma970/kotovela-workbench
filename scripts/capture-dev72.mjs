...
const outDir = path.resolve(process.env.SCREENSHOT_DIR ?? 'screenshots/dev72')
const evidenceDir = path.resolve(process.env.EVIDENCE_DIR ?? '.evidence/dev72')
...
const apiFixtures = {
  '/api/tasks-board': readJson('dev72-tasks-board-api.json'),
  '/api/leads': readJson('dev72-leads-api.json'),
  '/api/audit-log': readJson('dev72-audit-log-api.json'),
  '/api/system-mode': JSON.parse(fs.readFileSync(path.resolve(`server/data/system-mode.${mode}.json`), 'utf8')),
}

const shots = mode === 'internal'
  ? [
      ['/evidence-acceptance', 'DEV-72-internal-evidence-acceptance-overview.png', '[data-evidence-link="true"]'],
      ['/evidence-acceptance', 'DEV-72-internal-parser-fixture-dataset.png', '.consultant-evidence-list.top-gap'],
      ['/evidence-acceptance', 'DEV-72-internal-evidence-unresolved.png', '.evidence-unresolved-list'],
    ]
  : [
      ['/evidence-acceptance', 'DEV-72-opensource-evidence-isolation.png', '.page-note'],
    ]
...
await browser.close()
console.log(`Captured DEV-72 ${mode} screenshots at ${outDir}`)
