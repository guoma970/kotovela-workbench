...
const outDir = path.join(root, '.evidence', 'dev75')
...
const driftTrend = JSON.parse(fs.readFileSync(path.join(root, '.evidence', 'dev75', 'dev75-drift-trend.json'), 'utf8'))
...
fs.writeFileSync(path.join(outDir, 'dev75-tasks-board-api.json'), JSON.stringify(tasksPayload, null, 2))
fs.writeFileSync(path.join(outDir, 'dev75-leads-api.json'), JSON.stringify({ leads }, null, 2))
fs.writeFileSync(path.join(outDir, 'dev75-audit-log-api.json'), JSON.stringify(auditPayload, null, 2))
fs.writeFileSync(path.join(outDir, 'dev75-decision-log.json'), JSON.stringify({ entries: decisionEntries }, null, 2))
fs.writeFileSync(path.join(outDir, 'dev75-api-summary.json'), JSON.stringify(summary, null, 2))

console.log(`Prepared DEV-75 evidence in ${path.relative(root, outDir)}`)
