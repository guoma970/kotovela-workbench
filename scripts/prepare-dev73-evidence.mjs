...
const fixtureDataset = JSON.parse(fs.readFileSync(path.join(root, '.evidence', 'dev73', 'parser-fixture-dataset.json'), 'utf8'))
...
fs.writeFileSync(path.join(outDir, 'dev73-tasks-board-api.json'), JSON.stringify(tasksPayload, null, 2))
fs.writeFileSync(path.join(outDir, 'dev73-leads-api.json'), JSON.stringify({ leads }, null, 2))
fs.writeFileSync(path.join(outDir, 'dev73-audit-log-api.json'), JSON.stringify(auditPayload, null, 2))
fs.writeFileSync(path.join(outDir, 'dev73-decision-log.json'), JSON.stringify({ entries: decisionEntries }, null, 2))
fs.writeFileSync(path.join(outDir, 'dev73-api-summary.json'), JSON.stringify(summary, null, 2))

console.log(`Prepared DEV-73 evidence in ${path.relative(root, outDir)}`)
