...
const outDir = path.join(root, '.evidence', 'dev76')
...
const driftTrend = JSON.parse(fs.readFileSync(path.join(root, '.evidence', 'dev76', 'dev76-drift-trend.json'), 'utf8'))
...
const splitCoverage = {
  tasks_board: {
    total: board.length,
    with_account_line: board.filter((entry) => Boolean(entry.account_line)).length,
    with_source_line: board.filter((entry) => Boolean(entry.source_line)).length,
    with_content_line: board.filter((entry) => Boolean(entry.content_line)).length,
  },
  leads: {
    total: leads.length,
    with_account_line: leads.filter((entry) => Boolean(entry.account_line)).length,
    with_source_line: leads.filter((entry) => Boolean(entry.source_line)).length,
    with_content_line: leads.filter((entry) => Boolean(entry.content_line)).length,
  },
}
...
fs.writeFileSync(path.join(outDir, 'dev76-tasks-board-api.json'), JSON.stringify(tasksPayload, null, 2))
fs.writeFileSync(path.join(outDir, 'dev76-leads-api.json'), JSON.stringify({ leads }, null, 2))
fs.writeFileSync(path.join(outDir, 'dev76-audit-log-api.json'), JSON.stringify(auditPayload, null, 2))
fs.writeFileSync(path.join(outDir, 'dev76-decision-log.json'), JSON.stringify({ entries: decisionEntries }, null, 2))
fs.writeFileSync(path.join(outDir, 'dev76-api-summary.json'), JSON.stringify(summary, null, 2))

console.log(`Prepared DEV-76 evidence in ${path.relative(root, outDir)}`)
